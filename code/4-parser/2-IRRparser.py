...
def parser(irrf):
    ...
    for lineindex in range(len(lines)):
        ...
        #start line
        if line == '[Querying whois.radb.net]' or line == '[whois.radb.net]':
            last = 'start'
            continue
        #stop attributes
        stop = False
        ...
        if comm:
            ...
            if lower.lower() in stopwords:
                continue
            #Other communities in range xxx:xxx-xxx:xxx are accepted
            if line[:sp[0]].find('in range') > -1:
                continue

            #if the community value is a number
            if lower.isdigit():
                sentence = line[:sp[0]] + line[sp[1]:]
                sentence = sentence.strip()
                if sentence == '':
                    continue
                ...
                while i < len(lower):
                    ...
                    if c.isdigit():
                        ...
                        j = 0
                        while i < len(lower) and lower[i].isdigit() == False:
                            i += 1
                            j += 1
                        regexval += '[0-9]{'+str(j)+'}'
                regexval = '^'+regexval+'$'
                ...
                if sentence != '':
                    ...
                    res = rulebased(items)
                    if res:
                        assemble(dic,upper,regexval,'regular',res)
                # 12389:607y -> 12389:6071
                parts = []
                alphanum = 0
                alpha = ''
                i = 0
                while i < len(lower):
                    ...
                    if c.isdigit():
                        ...
                        j = i
                        while j < len(lower) and lower[j] == c:
                            j+=1
                ...
                if alphanum == 1:
                    explainline = 0
                    for lineindexj in range(lineindex+1,len(lines)):
                        explainline += 1
                        if explainline > 5:
                            break
                        linej = lines[lineindexj]
                        if linej.find(alpha) > -1 and linej.find('=') > -1:
                            nail = linej.find('=') 
                            explain = linej[nail+1:].strip()
                            # extact num in explain
                            ii = 0
                            numlist = []
                            while ii < len(explain):
                                c = explain[ii]
                                if not c.isdigit():
                                    ii += 1
                                    continue
                                    ...
                                    jj = ii
                                    while jj < len(explain) and explain[jj].isdigit():
                                        jj += 1
                                    if (jj-ii) == len(alpha):
                                        numlist.append(explain[ii:jj])
                                    ii = jj
                            valuelist = []
                            for num in numlist:
                                value = ''
                                for c in parts:
                                    if c != alpha:
                                        ...
                                        value+=num
                                valuelist.append([value,num])
                            if explain.find('prepend') > -1:
                                if explain.find('without') > -1 or explain.find('no') > -1:
                                    for value in valuelist:
                                        assemble(dic,upper,value[0],'explicit',[['prepend',0]])
                                else:
                                    for value in valuelist:
                                        if int(value[1]) < 10:
                                            assemble(dic,upper,value[0],'explicit',[['prepend',int(value[1])]])

                            else:
                                expsen = explain+' '+sentence
                                items = re.split('[ .()_\t/]',expsen)
                                res = rulebased(items)
                                if res:
                                    for value in valuelist:
                                        assemble(dic,upper,value[0],'explicit',res)


for fn in os.listdir('./IRR_Data/'):
    with open('./IRR_Data/'+fn,'r') as f:
        asn = fn[:-4]
        parser(f)


with open('semanticdic_irr.json','w') as f:
    json.dump(dic,f)
